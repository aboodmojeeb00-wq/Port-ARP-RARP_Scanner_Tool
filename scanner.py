...
def get_network_cidr(ip: str = None) -> str:
    """تخمين نطاق الشبكة المحلية (يفترض /24)"""
    if not ip:
        ip = get_local_ip()
    try:
        # افتراض شبكة /24 الشائعة
...
def ping_host(ip: str, timeout: int = 1) -> bool:
    """فحص حيوية المضيف باستخدام ping (يعمل على Windows و Linux)"""
    system = platform.system().lower()
    try:
        if system == "windows":
...
def arp_scan(network: str, timeout: int, max_threads: int, logger) -> list:
    ...
    # Wait for ARP table to update after pings
    time.sleep(1.5)
    arp_entries = get_arp_table(logger)

    # Prefer entries that match live hosts, but show all if available
    result = []
    live_set = set(live_hosts)
    for entry in arp_entries:
        result.append(entry)

    # If ARP table is empty but we found live hosts, show them with unknown MAC
    if not result and live_hosts:
        for ip in live_hosts:
            result.append({"ip": ip, "mac": "unknown", "type": "live"})
        logger.info("ARP table empty - showing live hosts with unknown MAC")
